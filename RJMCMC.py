...
class RJMCMC:
    ...
    def height_change_move(self, 
        ...
        # Choose which height to change (randomly select one of the k+1 heights)
        j = np.random.randint(0, k+1)

        # Propose new height from the prior distribution
        #h_j_prime = self.rate_prior.rvs()
            # propose new rate using log-normal random walk
        rem = 0.5
        u = np.random.uniform(-rem, rem)
        h_j = state[k + j]
        ...
        # log prior ratio (Gamma prior)
        log_prior_ratio = self.rate_prior.logpdf(h_j_prime) - self.rate_prior.logpdf(h_j)

        # Jacobian term
        log_jacobian = u

        log_accept_prob = log_like_ratio + log_prior_ratio + log_jacobian
    ...
    def birth_move(self, 
        ...
        # Prior on k: Poisson(lambda)
        log_prior_k_ratio = np.log(self.prior_k.pmf(k+1)) - np.log(self.prior_k.pmf(k))

        # # Prior on positions: even-numbered order statistics
        # # The prior is proportional to product of gaps: prod(g_j)
        # # For k change points, we have k+1 gaps
        # # For k+1 change points, we have k+2 gaps
        # # The new gap structure: g_j splits into g_j' and g_{j+1}'
        # # Old gaps product: g_j
        # # New gaps product: g_j' * g_{j+1}' where g_j' + g_{j+1}' = g_j
        # # So new/old = (g_j' * g_{j+1}') / g_j = (s_star - s_j) * (s_{j+1} - s_star) / (s_{j+1} - s_j)
        # # Also need to account for the normalization constant: L^{-2k-1} vs L^{-2(k+1)-1}
        log_prior_pos_ratio = np.log(s_star - s_j) + np.log(s_jplus1 - s_star) - \
                              np.log(s_jplus1 - s_j)

        # # Prior on heights: Gamma(alpha, beta)
       # C. Prior Ratio (Gamma(alpha, beta) for rates)
        # One hj leaves, h1 and h2 arrive.
        term1 = self.alpha * np.log(self.beta) - gammaln(self.alpha)
        term2 = (self.alpha - 1) * (np.log(h_j_prime) + np.log(h_jplus1_prime) - np.log(h_j))
        term3 = -self.beta * (h_j_prime + h_jplus1_prime - h_j)
        log_prior_h_ratio = term1 + term2 + term3
        
        # Dirichlet normalisation
        log_dirichlet_norm = np.log(2*(k+1)) + np.log(2*(k+3)) - 2*np.log(self.duration)

        # log_prior_ratio = log_prior_k_ratio + log_prior_pos_ratio + log_prior_h_ratio

        log_prior_ratio = log_prior_k_ratio + log_prior_h_ratio + log_prior_pos_ratio + log_dirichlet_norm
    ...
    def death_move(self, 
        ...
        # # Prior ratio = P(k-1) * P(positions_{k-1}) * P(heights_{k-1}) / [P(k) * P(positions_k) * P(heights_k)]
        # # This is the inverse of the birth move
        # # Prior on k: Poisson(lambda)
        log_prior_k_ratio = np.log(self.prior_k.pmf(k-1)) - np.log(self.prior_k.pmf(k))

        # # Prior on positions: even-numbered order statistics
        # # We're merging two gaps into one: g_j' = g_{j-1} + g_j
        # # Old gaps product: g_{j-1} * g_j (where g_{j-1} = s_j - s_{j-1}, g_j = s_{j+1} - s_j)
        # # New gap: g_j' = s_{j+1} - s_{j-1}
        # # Ratio = g_j' / (g_{j-1} * g_j) + normalization
        gap_denom = (s_j - s_jminus1) * (s_jplus1 - s_j) / (s_jplus1 - s_jminus1)
        log_prior_pos_ratio = (
            2 * np.log(self.duration) - 
            np.log(2*k) - np.log(2*k + 1) -  np.log(gap_denom))
        

        # # Prior on heights: Gamma(alpha, beta)
        # # We're replacing h_{j-1} and h_j with h_{j-1}' (one height instead of two)
        # # P(h_{j-1}') / [P(h_{j-1}) * P(h_j)]
        h_j = poisson_rates[j-1]
        h_jplus1 = poisson_rates[j]
        h_prime = proposed_poisson_rates[j-1]
        
        # Normalization constant for one Gamma(alpha, beta) distribution
        # Note: We subtract it because we are reducing the dimension by 1
        log_gamma_norm = self.alpha * np.log(self.beta) - gammaln(self.alpha)
        
        log_prior_h_ratio = (
            ((self.alpha - 1) * np.log(h_prime) - self.beta * h_prime) - 
            ((self.alpha - 1) * np.log(h_j) - self.beta * h_j) -
            ((self.alpha - 1) * np.log(h_jplus1) - self.beta * h_jplus1) -
            log_gamma_norm
        )
        log_prior_ratio = log_prior_k_ratio + log_prior_pos_ratio + log_prior_h_ratio
